...
Each backend module has a single, narrow responsibility and no knowledge of how the others work internally — `embeddings.py` doesn't know where text comes from, `vector_store.py` doesn't know how embeddings are computed, etc. This keeps each piece independently testable and easy to reason about.
...
Create a `.env` file (see `.env.example` for the required keys — you'll need your own Voyage AI, Chroma Cloud, and Anthropic API keys):
...
- **Uploaded files aren't stored permanently.** Files are processed in a temp location just long enough to extract text, then discarded — the vector store already holds everything needed to answer questions, so there's no dependency on persistent local disk (which most free hosting tiers don't offer anyway).
- **Embeddings are API-based, not local.** Running an embedding model locally (e.g. sentence-transformers) would need far more RAM than a free-tier host typically provides. An API-based embedding service keeps the deployed app lightweight, at a small per-call cost.
- **Chunk size (800 chars) and overlap (120 chars)** are tunable in `document_loader.py` — smaller chunks retrieve more precisely but lose surrounding context; larger chunks do the opposite.
